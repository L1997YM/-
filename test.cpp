...
int main()
{
	...
	while (!glfwWindowShouldClose(window))		//让GLFW退出前一直保持运行
	{
		...
		if (isPhong && (!isGouraud))
		{
			ImGui::Begin("orthographic");
			ImGui::SliderFloat("left", (float*)&left,-20.0f,0.0f);
			ImGui::SliderFloat("right", (float*)&right, 0.0f, 15.0f);
			ImGui::SliderFloat("top", (float*)&top, -0.0f, 15.0f);
			ImGui::SliderFloat("bottom", (float*)&bottom, -15.0f, 0.0f);
			ImGui::SliderFloat("near", (float*)&nears, 0.0f, 100.0f);
			ImGui::SliderFloat("far", (float*)&fars, 0.0f, 100.0f);
			ImGui::End();
		}
		if (isGouraud && (!isPhong))
		{
			ImGui::Begin("perspective");
			ImGui::SliderFloat("radian", (float*)&rad, 0.0f, 90.0f);
			ImGui::SliderFloat("near", (float*)&nears, 0.0f, 100.0f);
			ImGui::SliderFloat("far", (float*)&fars, 0.0f, 100.0f);
			ImGui::End();
		}*/
